...
#        self.P=np.zeros((self.Ny, self.Nx))
    ...
    def CV_vol(self):
        v=np.zeros_like(self.eta)
        dx,dy=np.meshgrid(self.dx, self.dy)
        v[1:-1,1:-1]=self.X[1:-1,1:-1]*0.25*(dx[1:-1,1:-1]+dx[1:-1,:-2])*(dy[1:-1,1:-1]+dy[:-2,1:-1])
        v[0,1:-1]   =self.X[0,1:-1]*0.25*(dx[0,1:-1]+dx[0,:-2])*(dy[0,1:-1])
        v[0,-1]     =self.X[0,-1]*0.25*(dx[0,-1])*(dy[0,-1])
        v[-1,1:-1]  =self.X[-1,1:-1]*0.25*(dx[-1,1:-1]+dx[-1,:-2])*(dy[-1,1:-1])
        v[1:-1,-1]  =self.X[1:-1,-1]*0.25*(dx[1:-1,-1])*(dy[1:-1,-1]+dy[:-2,-1])
        v[-1,-1]    =self.X[-1,-1]*0.25*(dx[-1,-1])*(dy[-1,-1])
        v[:,0]      =0
        return v
